...
BridgeConfigPath = os.environ.get('DBUS_MQTT_PATH') or '/data/conf/mosquitto.d/vrm_bridge.conf'
MosquittoConfigPath = os.path.join(AppDir, 'mosquitto.conf')
...
class DbusMqtt(object):
	...
	def _init_broker(self):
		if os.path.exists(BridgeConfigPath):
		...
		try:
			...
			with open(BridgeConfigPath, 'wt') as out:
	...
	def _start_broker(self):
		# @todo EV When mosquitto becomes a regular daemon, we should restart it here ('service mosquitto
		# restart'), but only if the configuration has been changed.
		logging.info('Starting broker')
		subprocess.Popen(['/usr/sbin/mosquitto', '-c', MosquittoConfigPath], stdout=subprocess.PIPE)
